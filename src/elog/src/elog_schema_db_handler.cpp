...
ELogTarget* ELogSchemaDbHandler::loadTarget(const std::string& logTargetCfg,
    ...
    ELogPropertyMap::const_iterator itr = std::find_if(
        targetSpec.m_props.begin(), targetSpec.m_props.end(),
        [](const ELogProperty& prop) { return prop.second.compare("conn-string") == 0; });
    if (itr == targetSpec.m_props.end()) {
        ELOG_ERROR("Invalid database log target specification, missing property conn-string: %s",
                   logTargetCfg.c_str());
        return nullptr;
    }
    ...
    itr = std::find_if(
        targetSpec.m_props.begin(), targetSpec.m_props.end(),
        [](const ELogProperty& prop) { return prop.second.compare("insert-query") == 0; });
    if (itr == targetSpec.m_props.end()) {
        ELOG_ERROR("Invalid database log target specification, missing property insert-query: %s",
                   logTargetCfg.c_str());
        return nullptr;
    }
    ...
    if (dbType.compare("mysql") == 0) {
        return ELogMySqlDbHandler::loadTarget(logTargetCfg, connString, insertQuery);
    }
#endif
#ifdef ELOG_ENABLE_POSTGRESQL_DB_CONNECTOR
    if (dbType.compare("postgresql") == 0) {
        return ELogPostgreSqlDblHandler::loadTarget(logTargetCfg, connString, insertQuery);
    }
#endif
#ifdef ELOG_ENABLE_ORACLE_DB_CONNECTOR
    if (dbType.compare("oracle") == 0) {
        return ELogOracleDbHandler::loadTarget(logTargetCfg, connString, insertQuery);
    }
#endif
#ifdef ELOG_ENABLE_SQLSERVER_DB_CONNECTOR
    if (dbType.compare("sqlserver") == 0) {
        return ELogSqlServerDbHandler::loadTarget(logTargetCfg, connString, insertQuery);
    }
#endif

    ELOG_ERROR("Invalid database log target specification, unsupported db type %s: %s",
